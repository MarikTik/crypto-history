...
class OHLCV_Database(WriteOnlyDatabase):
    ...
    Attributes:
        TEMPORARY_FILE_COMPRESSION_THRESHOLD (int): The maximum size (in bytes)
    ...
    TEMPORARY_FILE_COMPRESSION_THRESHOLD = 10 * 1024**2  # 10 Mb
    ...
    def write(
        ...
        This method:
         ...
         3. Checks if the CSV file size exceeds the threshold (`TEMPORARY_FILE_COMPRESSION_THRESHOLD`).
        ...
        if (
            ...
            > OHLCV_Database.TEMPORARY_FILE_COMPRESSION_THRESHOLD
        ):
    ...
    def _compress(self, product: str):
        ...
        if parquet_path.exists():  # Merge with existing parquet data
            ...
            prev_df["timestamp"] = pd.to_datetime(
                prev_df["timestamp"], utc=True
            )
    ...
    def close(self) -> None:
        ...
        if self._last_used_path is not None:
            if (
                self._last_used_path.exists()
                and self._last_used_path.stat().st_size > 0
            ):
