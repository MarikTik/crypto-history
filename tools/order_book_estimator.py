ENTRY_SIZE = 16  # 8 bytes price + 8 bytes quantity
TIMESTAMP_SIZE = 4  # 4 bytes for timestamp
KEY_SIZE = 2  # 4 bytes for reference pointer to the coin type
SQLITE_METADATA_FACTOR = 1.1
...
SNAPSHOT_SIZE = (
    ENTRY_SIZE * depth * 2 + TIMESTAMP_SIZE + KEY_SIZE
) * SQLITE_METADATA_FACTOR

assets = int(input("Enter amount of assets to track: "))
months = float(
    input("Enter number of months to run (decimal number is valid): ")
)
...
print(
    f"📉 Compressed (×{compression_rate:.2f}): {storage_required_mb / compression_rate:.2f} MB ({storage_required_gb / compression_rate:.2f} GB)"
)
